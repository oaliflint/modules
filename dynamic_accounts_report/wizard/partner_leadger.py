...
class PartnerView(models.TransientModel):
    ...
    def _get_partners(self, partners, accounts, init_balance, display_account, data):
        ...
        for partner in partners:
            ...
            if data.get('date_from'):
                res['Init_balance'] = self.get_init_bal(partner, accounts, data)
    ...
    def get_init_bal(self, partner, accounts, data):
        ...
        if data.get('date_from'):
            ...
            WHERE += ' AND p.id = %s' % (partner.id)
            if data.get('reconciled') == 'unreconciled':
                WHERE += ' AND l.full_reconcile_id is null AND' \
                         ' l.balance != 0 AND a.reconcile is true'

            sql = ('''SELECT l.partner_id AS partner_id, 
                    SUM(COALESCE(l.debit,0)) AS debit, SUM(COALESCE(l.credit,0)) AS credit, 
                    (SUM(COALESCE(l.debit,0)) - SUM(COALESCE(l.credit,0))) AS balance
                               FROM account_move_line l\
                               JOIN account_move m ON (l.move_id=m.id)\
                               JOIN account_account a ON (l.account_id=a.id)
                               LEFT JOIN res_currency c ON (l.currency_id=c.id)\
                               ...
                               JOIN account_journal j ON (l.journal_id=j.id)\
                               JOIN account_account acc ON (l.account_id = acc.id) '''
                   + WHERE + new_final_filter + ''' GROUP BY l.partner_id''')
            if data.get('accounts'):
                params = tuple(where_params)
            else:
                params = (tuple(accounts.ids),) + tuple(where_params)
