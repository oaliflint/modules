...
class GeneralView(models.TransientModel):
    ...
    def _get_report_values(self, data):
        ...
        debit_balance = round(debit_total, 3) - round(credit_total, 3)
        return {
            'doc_ids': self.ids,
    ...
    def _get_accounts(self, accounts, init_balance, display_account, data):
        ...
        sql = ('''SELECT l.account_id AS account_id, a.code AS code,a.id AS id, a.name AS name, ROUND(COALESCE(SUM(l.debit),0),3) AS debit, ROUND(COALESCE(SUM(l.credit),0),3) AS credit, ROUND(COALESCE(SUM(l.balance),0),3) AS balance
        ...
        print(account_res,'resssssssssssssssssssssss')
