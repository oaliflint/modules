...
class KsGlobalDiscountInvoice(models.Model):
    ...
    ks_global_discount_type = fields.Selection([
        ...
        states={'draft': [('readonly', False)],
                'sent': [('readonly', False)]},
        default='percent')
    amount_before_discount = fields.Monetary(string='Total Before Discount', readonly=True,
                                             digits=dp.get_precision('Account'))
    amount_after_discount = fields.Monetary(string='Total after Discount', readonly=True,
                                            digits=dp.get_precision('Account'))
    ...
    total_discount_per_line = fields.Monetary(string='Discount ',
                                              readonly=True,
                                              compute='_compute_amount',
                                              store=True, track_visibility='always')
    ...
    def _compute_amount(self):
        ...
        for rec in self:
            ...
            if rec.invoice_line_ids:
                amount_discount = sum(rec.invoice_line_ids.mapped('amount_discount'))
                rec.total_discount_per_line = amount_discount
                rec.amount_before_discount = sum(
                    rec.invoice_line_ids.mapped('price_subtotal')) + amount_discount
                rec.amount_after_discount = sum(rec.invoice_line_ids.mapped(
                    'price_subtotal'))
    ...
    def ks_update_universal_discount(self):
        ...
        for rec in self:
            ...
            if already_exists:
                ...
                if amount == 0:
                    rec.line_ids = [(3, already_exists.id)]
                else:
    ...
    @api.onchange('ks_global_discount_rate', 'ks_global_discount_type', 'line_ids','invoice_line_ids')
            ...
            if rec.ks_global_discount_rate > 0 and rec.move_type in type_list:
                if rec.is_invoice(include_receipts=True):
                    ...
                    if already_exists:
                        ...
                        print('insideeeeeeeeeeeeeeeeee',self.ks_sales_discount_account_id)
                        if self.ks_sales_discount_account_id \
                                and (self.move_type == "out_invoice"
                                     or self.move_type == "out_refund"):
                            amount = self.ks_amount_discount
